...
-- 2017-01-01,RChoi,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_RChoi')
	 Begin 
	  Alter Database [Assignment06DB_RChoi] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_RChoi;
	 End
	Create Database Assignment06DB_RChoi;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_RChoi;
...
SELECT 
    ...
    B.UnitPrice
FROM dbo.Products B
JOIN dbo.Categories A ON B.CategoryID = A.CategoryID;
GO
...
SELECT 
    ...
    B.InventoryDate,
    B.[Count]
FROM dbo.Inventories B
JOIN dbo.Products A ON B.ProductID = A.ProductID;
GO
...
SELECT 
    A.InventoryDate,
    MIN(B.EmployeeFirstName + ' ' + B.EmployeeLastName) AS EmployeeName
FROM dbo.Inventories A
JOIN dbo.Employees B ON A.EmployeeID = B.EmployeeID
GROUP BY A.InventoryDate;
GO
